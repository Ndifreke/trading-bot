...
// func getStableSideTrader_v1(tradeConfigs []names.TradeConfig, bestSide names.TradeSide, status status, retrade stableTraderCreator, bestConfig names.TradeConfig) *stableBestSide_v1 {
...
// func (tm *stableBestSide_v1) Done(bestConfig names.TradeConfig, locker names.LockInterface) {
...
// func createStableTrader(configs []names.TradeConfig, bestSide names.TradeSide, status status, tradeCreator stableTraderCreator, bestConfig names.TradeConfig) *manager.TradeManager {
...
// 	stableBestSide := getStableSideTrader_v1(preparedConfig, bestSide, status, tradeCreator, updatedBestConfig)
// 	return manager.NewTradeManager(stableBestSide)
// }

// // bestSide the side that the contention will fall to after the parallel side finds a candidate
// func NewStableBestSide_v1(configs []names.TradeConfig, bestSide names.TradeSide, status status, bestConfig names.TradeConfig) *manager.TradeManager {
...
func (t *stableBestSide_v1) RemoveConfig(config names.TradeConfig) bool {
	...
	// for _, tc := range t.contentionConfigs {
	// 	if tc == config {
	// 		//TODO COMPARE ID WHEN STABLE
	// 		// We should also remove this from the locker
	// 		removed = t.broadcast.Unsubscribe(config)
	// 	} else {
	// 		updatedConfigs = append(updatedConfigs, tc)
	// 	}
